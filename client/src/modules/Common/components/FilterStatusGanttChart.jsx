...
function FilterStatusGanttChart() {
  ...
  const { childFilterName, parentFilterName } = useSelector(
    (state) => state.common
  );
  ...
  const QUERY_KEY_CLIENTS_FILTER = ["gantt_filter_client_project", false];

  //filter_client_project
  const { data, isLoading, isFetching } = useQuery({
    queryKey: QUERY_KEY_CLIENTS_FILTER,
    queryFn: () => {
      return handleGetClientsWithProjectsFilter(organization_id, false, false);
    },
...
export default React.memo(FilterStatusGanttChart);
